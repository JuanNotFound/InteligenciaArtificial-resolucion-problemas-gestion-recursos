...
class Branch_and_bound:
    ...
    def iniciar(n_tarea: int, n_recurso: int, duraciones: [int], recursos: [int], dependencias) -> [int]:
        solucion = Branch_and_bound.algoritmo(n_tarea, n_recurso, duraciones, recursos, dependencias)
    ...
    def algoritmo(n_tarea: int, n_recurso: int, duraciones: [int], recursos: [int], dependencias) -> Nodo:
        ...
        while not salir:
            ...
            lista_nodos = sorted(lista_nodos, key=lambda x: x.calcular_makespan(duraciones), reverse=True)
